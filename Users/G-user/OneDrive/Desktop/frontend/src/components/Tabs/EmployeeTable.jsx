...
import { getAllEmployees, toggleEmployeeStatus, updateEmployee } from "../../api"; // تأكد من المسار الصحيح
...
const openEditModal = (employee) => {
  ...
  const handleRoleChange = async (id, newRole) => {
    try {
      await updateEmployee(id, newRole);
      setEmployees((prev) =>
...
<td className="table-text">
  ...
  >
    ...
    />
      ...
      {isEditModalOpen && (
  <AddEmployeeModel
    employeeData={selectedEmployee} // نمرر بيانات الموظف
    onClose={() => setIsEditModalOpen(false)}
    type="edit" // لو بدك تفرقيه عن الإضافة داخل المودال نفسه
  />
)}
