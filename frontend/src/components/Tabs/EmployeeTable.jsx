...
useEffect(() => {
  const fetchData = async () => {
    try {
      ...
      const employeesList = data.data?.employees || data.employees || data || [];

      setEmployees(employeesList);
      setTotalEmployees(employeesList.length);

      //   حفظهم محليًا لتسريع المرات القادمة
...
const handleDeleteEmployee = async (id) => {
  ...
  className="p-2 rounded-full hover:bg-purple-100 hover:scale-110 transition transform duration-200 cursor-pointer"
>
...
<button
  ...
  className="p-2 rounded-full hover:bg-red-100 hover:scale-110 transition transform duration-200 cursor-pointer"
>
