...
const sanitizeMembers = (members = []) =>
  (Array.isArray(members) ? members : []).filter(Boolean).map(sanitizeMember);
...
export default function SubscribersTab({
  ...
  useEffect(() => {
    try {
      ...
      console.error(
        "[SubscribersTab] فشل قراءة currentUser من localStorage:",
        err
      );
    }
  }, []);
  ...
  // جلب جميع المشتركين (مع دعم السيرتش من الباك + الفلاتر)
  useEffect(() => {
    const hasFilters =
      filters &&
    ...
    // بس لما ما يكون في سيرتش ولا فلاتر
    let localMembers = [];
    ...
    const fetchAllMembers = async () => {
      ...
      try {
        ...
        while (hasMore) {
          const data = await getAllMembers(currentPage, searchValue, filters);
        ...
        const mergedMembersRaw = allMembers.map((m) =>
          m && m._id && localById[m._id] ? { ...localById[m._id], ...m } : m
        );
        ...
        // نخزن في localStorage فقط لو مش سيرتش ولا فلاتر
        if (!searchValue && !hasFilters) {
  ...
  // إبلاغ البيرنت بعدد المشتركين المعرُوضين حاليًا (بعد فلترة/سيرتش/حذف/إضافة)
  useEffect(() => {
    if (typeof onMembersCountChange === "function") {
      onMembersCountChange(clients.length);
    }
