...
export default function ParticipantsSelector({ selectedParticipants, setSelectedParticipants, membersList }) {
  ...
  return (
    <div className="relative w-full">
      ...
      >
        <span className={`h-10 w-full flex items-center pl-2 ${selectedParticipants.length ? "text-black" : "text-gray-400"} font-normal`}>
          {selectedParticipants.length > 0 ? `${selectedParticipants.length} مشتركين` : "اختر العدد"}
        </span>
        <img src={addcircleIcon} alt="addcircle" className="w-4 h-4" />
      </div>
              ...
              .map((member, idx) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => {
                      let updated;
                      if (isSelected) updated = selectedParticipants.filter((m) => m !== member);
                  ...
                  >
                    ...
                    <div className={`w-5 h-5 border-2 rounded-full flex items-center justify-center ${isSelected ? "bg-purple-500 border-purple-500" : "border-gray-400 bg-white"}`}>
                      {isSelected && (
